...
from apps.usuarios.serializers import UserTokenSerializer
            ...
            if user.is_active:
                ...
                if created:
                    ...
                    return Response({
                        ...
                        'message':'Inicio de Sesion Exitoso'},status=status.HTTP_201_CREATED)"""
                    token.delete()
                    return Response({'error':'ya se ha iniciado session con este usuario'},status=status.HTTP_409_CONFLICT)
            else:
    ...
    def post(self,request,*args,**kwargs):
        try:
            ...
            if token:
                ...
                return Response({'token_message':
                token_message,
                session_message:session_message},status=status.HTTP_200_OK)
